...
import {isCalculatingTimeZone} from './timeZone.js';
import {getMapTimeZone, setTimeZoneLookupEnabled} from './map.js';
...
const timeZones = [
    ...
    ['DFT', 'AIX equivalent of Central European Time', '+1'],
...
let customTimeZoneOffset = 0;

let blankIndex = -1;

const timeZoneModalLink = document.getElementById('time-zone-link');
const timeZoneModal = new bootstrap.Modal(document.getElementById('time-zone-modal'));

const localTimeZoneModalRadio = document.getElementById('local-time-zone-modal-radio');
const mapTimeZoneModalRadio = document.getElementById('map-time-zone-modal-radio');
const customTimeZoneModalRadio = document.getElementById('custom-time-zone-modal-radio');

let timeZoneModalSelection = 'local';

const localTimeZoneModalLabel = document.getElementById('local-time-zone-modal-label');
const mapTimeZoneModalLabel = document.getElementById('map-time-zone-modal-label');

const timeZoneModalApplyButton = document.getElementById('time-zone-modal-apply-button');

function calculateLocalTimeZoneOffsetMins () {
...
function calculateLocalTimeZoneOffsetHours () {

    return (calculateLocalTimeZoneOffsetMins() / MINUTES_IN_HOUR);

}

function getTimeZoneMode () {

    return timeZoneModalSelection;

}

function getTimeZoneTextFromOffset (offsetHours) {
    ...
    const timeZoneOffset = Math.abs(offsetHours);

    const timeZoneOffsetHours = Math.floor(timeZoneOffset);
    const timeZoneOffsetMins = Math.abs(timeZoneOffset - timeZoneOffsetHours) * MINUTES_IN_HOUR;

    if (timeZoneOffset !== 0) {

        timeZoneText += isNegative ? '-' : '+';
...
function formatTimeZoneString (str) {
...
function getLocalTimeZoneString () {
    ...
    const str = getTimeZoneTextFromOffset(calculateLocalTimeZoneOffsetHours());
    return (formatTimeZoneString(str));

}

function getMapTimeZoneString () {

    const str = getTimeZoneTextFromOffset(getMapTimeZone());
    return (formatTimeZoneString(str));

}

function getCustomTimeZoneOffsetString () {

    return formatTimeZoneString(getTimeZoneTextFromOffset(customTimeZoneOffset));

}

function getSelectedTimeZoneOffsetMins () {

    switch (timeZoneModalSelection) {

    case 'local':
        return calculateLocalTimeZoneOffsetHours() * MINUTES_IN_HOUR;
    case 'map':
        return getMapTimeZone() * MINUTES_IN_HOUR;
    case 'custom':
        return customTimeZoneOffset * MINUTES_IN_HOUR;
    default:
        return calculateLocalTimeZoneOffsetHours() * MINUTES_IN_HOUR;
...
function updateMapTimeZoneModalLabel () {

    mapTimeZoneModalLabel.innerText = getMapTimeZoneString();

}

function updateRadioButtonUI () {

    if (localTimeZoneModalRadio.checked) {

        localTimeZoneModalLabel.classList.remove('disabled-time-zone-option-text');
        mapTimeZoneModalLabel.classList.add('disabled-time-zone-option-text');

        timeZoneSelect.disabled = true;
        subtractHourButton.classList.add('disabled');
        subtractMinButton.classList.add('disabled');
        offsetDisplay.classList.add('disabled-time-zone-option-text');
        addHourButton.classList.add('disabled');
        addMinButton.classList.add('disabled');

    } else if (mapTimeZoneModalRadio.checked) {

        localTimeZoneModalLabel.classList.add('disabled-time-zone-option-text');
        mapTimeZoneModalLabel.classList.remove('disabled-time-zone-option-text');
    ...
    } else if (customTimeZoneModalRadio.checked) {

        localTimeZoneModalLabel.classList.add('disabled-time-zone-option-text');
        mapTimeZoneModalLabel.classList.add('disabled-time-zone-option-text');

        timeZoneSelect.disabled = false;
        subtractHourButton.classList.remove('disabled');
        subtractMinButton.classList.remove('disabled');
        offsetDisplay.classList.remove('disabled-time-zone-option-text');
        addHourButton.classList.remove('disabled');
        addMinButton.classList.remove('disabled');

    }

}

localTimeZoneModalRadio.addEventListener('change', updateRadioButtonUI);
mapTimeZoneModalRadio.addEventListener('change', updateRadioButtonUI);
customTimeZoneModalRadio.addEventListener('change', updateRadioButtonUI);

function updateButtons () {

    subtractHourButton.disabled = customTimeZoneOffset <= -12 * MINUTES_IN_HOUR;
    subtractMinButton.disabled = customTimeZoneOffset <= -12 * MINUTES_IN_HOUR;

    addHourButton.disabled = customTimeZoneOffset >= 14 * MINUTES_IN_HOUR;
    addMinButton.disabled = customTimeZoneOffset >= 14 * MINUTES_IN_HOUR;

}

function updateOffsetDisplay () {

    offsetDisplay.innerText = getTimeZoneTextFromOffset(customTimeZoneOffset);

}

function updateTimeZoneSelect () {

    if (customTimeZoneOffset === 0) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        if (timeZoneOptionValue === customTimeZoneOffset) {

            timeZoneSelect.value = customTimeZoneOffset.toString();
...
function checkOffsetValueRange () {
    ...
    customTimeZoneOffset = Math.max(customTimeZoneOffset, -12);
    customTimeZoneOffset = Math.min(customTimeZoneOffset, 14);

}

function updateOffset (diff) {

    customTimeZoneOffset += diff;
...
subtractMinButton.addEventListener('click', () => {
    ...
    updateOffset(-15 / MINUTES_IN_HOUR);
...
function addBlankOption () {
    ...
    blankOpt.value = customTimeZoneOffset.toString();

    blankOpt.innerHTML = getTimeZoneTextFromOffset(customTimeZoneOffset);

    for (let i = 0; i < options.length; i++) {

        const timeZoneOptionValue = parseInt(options[i].value);

        if (customTimeZoneOffset <= timeZoneOptionValue) {
...
for (let i = 0; i < timeZones.length; i++) {
    ...
    const timeZoneOffset = timeZoneOffsetHours < 0 ? timeZoneOffsetHours - (timeZoneOffsetMin / MINUTES_IN_HOUR) : timeZoneOffsetHours + (timeZoneOffsetMin / MINUTES_IN_HOUR);

    /* Generate option text */

    const optionText = getTimeZoneTextFromOffset(timeZoneOffset) + '   ' + acronym + '   ' + name;
...
timeZoneSelect.addEventListener('change', () => {
    ...
    customTimeZoneOffset = timeZoneSelect.value;
...
timeZoneModalLink.addEventListener('click', () => {

    if (isCalculatingTimeZone()) {
    ...
    case 'local':
        localTimeZoneModalRadio.checked = true;
        break;
    case 'map':
        mapTimeZoneModalRadio.checked = true;
        break;
    case 'custom':
        customTimeZoneModalRadio.checked = true;
        break;
    default:
        localTimeZoneModalRadio.checked = true;
        timeZoneModalSelection = 'local';
...
timeZoneModalApplyButton.addEventListener('click', () => {

    timeZoneModalSelection = localTimeZoneModalRadio.checked ? 'local' : (mapTimeZoneModalRadio.checked ? 'map' : 'custom');

    updateTimeZone();

    setTimeZoneLookupEnabled(timeZoneModalSelection === 'map');

    timeZoneModal.hide();

});

export {getTimeZoneMode, updateMapTimeZoneModalLabel, getLocalTimeZoneString, getMapTimeZoneString, getCustomTimeZoneOffsetString, getSelectedTimeZoneOffsetMins};
